  ...
  : BaseLoader(path, comm), _datafile(DcmFileFormat())
{
  ...
  OFCondition status = _datafile.loadFile(path.c_str(), EXS_Unknown, EGL_noChange,
                                         METADATA_MAX_BYTES, ERM_autoDetect);
...
void DCMLoader::copy_scaled_chunk(floating ***data, const intvector &size, 
                                  const intvector &offset) const
{
  DcmDataset *dataset = this->_datafile.getDataset();
  DicomImage img(dataset, EXS_Unknown, (unsigned long)(0), (unsigned long)offset[2],
                 (unsigned long)size[2]);  //NB frames in img are reindexed from the first loaded frame

  uinteger bitdepth = img.getDepth();
  double dmin, dmax;
  img.getMinMaxValues(dmin, dmax, 1); //mode 1 gets absolute min/max, 0 gets actual from loaded frames
