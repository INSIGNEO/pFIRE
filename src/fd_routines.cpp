...
void fd::gradient_existing(const DM &dmda, const Vec &srcvec, Vec &tgtvec, integer dim)
{
  ...
  // Recall grad and image share a DMDA
  floating ***img_array, ***grad_array; //acceptable to use raw ptrs here as memory handled by PETSc
  perr = DMDAVecGetArray(dmda, srcvec, &img_array);CHKERRABORT(PETSC_COMM_WORLD, perr);
  perr = DMDAVecGetArray(dmda, tgtvec, &grad_array);CHKERRABORT(PETSC_COMM_WORLD, perr);
