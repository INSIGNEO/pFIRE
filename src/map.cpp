...
std::unique_ptr<Map> Map::interpolate(const floatvector& new_spacing)
{
  ...
  Mat_unique interp = build_basis_matrix(m_comm, map_shape, new_map->map_shape,
...
void Map::initialize_dmda() const
{
  ...
  perr = DMDACreate3d(m_comm,
                      DM_BOUNDARY_NONE, DM_BOUNDARY_NONE, DM_BOUNDARY_NONE, //BCs
                      ...
                      map_dmda.get());CHKERRABORT(m_comm, perr);

  perr = DMSetUp(*(map_dmda));CHKERRABORT(m_comm, perr);
}
...
std::pair<intvector, intvector> Map::get_dmda_local_extents() const
{
  ...
  PetscErrorCode perr = DMDAGetCorners(*map_dmda, &locs[0], &locs[1], &locs[2],
...
Vec_unique Map::get_dim_data_dmda_blocked(integer dim) const
{
  ...
  PetscErrorCode perr = DMDAGetAO(*map_dmda, &ao_petsctonat);// Destroying this would break the dmda
  CHKERRABORT(m_comm, perr);
  ...
  perr = VecGetOwnershipRange(*tmp_data, &startelem, &datasize);CHKERRABORT(m_comm, perr);
  datasize -= startelem;
...
void Map::calculate_basis()
{
  ...
  m_basis = build_basis_matrix(m_comm, map_shape, m_v_image_shape,
                               scalings, offsets, m_ndim, m_ndim+1);
